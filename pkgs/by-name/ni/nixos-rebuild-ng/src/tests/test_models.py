...
def test_build_attrset_from_arg() -> None:
    assert m.BuildAttrset.from_arg(None, None) is None
    assert m.BuildAttrset.from_arg(None, "attr") == m.BuildAttrset(
        Path("default.nix"), "attr"
    )
    assert m.BuildAttrset.from_arg("file.nix", "attr") == m.BuildAttrset(
        Path("file.nix"), "attr"
    )
    assert m.BuildAttrset.from_arg("file.nix", None) == m.BuildAttrset(
        Path("file.nix"), None
    )


def test_build_attrset_to_attr() -> None:
    assert (
        m.BuildAttrset(Path("file.nix"), None).to_attr("attr1", "attr2")
        == "attr1.attr2"
    )
    assert (
        m.BuildAttrset(Path("file.nix"), "preAttr").to_attr("attr1", "attr2")
        == "preAttr.attr1.attr2"
    )


def test_build_module_to_attr() -> None:
    assert m.BuildModule().to_attr("attr1", "attr2") == "attr1.attr2"
