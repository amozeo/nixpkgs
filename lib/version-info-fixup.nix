...

{
  trivial = prevLib.trivial // {
    versionSuffix = if finalLib.isFunction versionSuffix then versionSuffix finalLib else versionSuffix;
    revisionWithDefault =
      if finalLib.isFunction revision then
        revision
      else if finalLib.isString revision then
        _: revision
      else
        default: default;
  };
